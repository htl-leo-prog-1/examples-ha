...
void Initialize()
{
    Board.Init(ROWS, COLS, "Draw a house");
}

void DrawDot(int x, int y, Color color)
{
    Board.SetText(ROWS-1-y, x, "@",color.Name);
}
...
void DrawBox(int x0, int y0, int x1, int y1, Color color)
{
...
void FillBox(int x0, int y0, int x1, int y1, Color color)
{
...
void DrawAWindow(int x, int y, Color color)
{
    FillBox(x, y, x + 4, y + 2, color);
}

void DrawADoor(int x, int y, Color color)
{
    FillBox(x, y+1, x + 4, y + 4, color);
}

void DrawAHouse(int x, int y, Color color)
{
    DrawADoor(x + 3, y + 0, Color.DarkRed);
    DrawAWindow(x + 10, y + 2, Color.Gray);
    DrawAWindow(x + 3, y + 6, Color.Gray);
    DrawAWindow(x + 10, y + 6, Color.Gray);
...
DrawLine(0, 0, COLS - 1, 0, Color.Green);
DrawAHouse(5, 1, Color.White);
DrawAHouse(30, 2, Color.Magenta);
DrawAHouse(55, 3, Color.Yellow);
