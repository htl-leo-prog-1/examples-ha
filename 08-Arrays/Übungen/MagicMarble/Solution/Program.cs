var marbels = new[]
{
    ...
    ">>>0x0014<<<>>>0023<<<>>",
    ">>>>0028<<<>>0031<<<>>>0012<<<>>",
    ">>>0x0015<<<<>>>>0026<<<>>>>0x002e<<>>>>0048<<<<>>0x003e<<<>>>>",
...
foreach (string marble in marbels)
{
    int idx = 0;
    int teleports = 0;
    int segments = 0;
    ...
    Console.WriteLine();
    Console.WriteLine($"Marble={marble.Substring(0, Math.Min(30, marble.Length))}...");

    if (IsValidMarbleRun(marble))
    {
        while (idx < marble.Length)
        {
            idx = GetNextIndex(marble, idx, ref segments, ref teleports);
        }

        Console.WriteLine($"Segements={segments}, Teleports={teleports}");
    }
    else
    {
...
bool IsValidMarbleRun(string marble)
{
    int idx = 0;
    int teleports = 0;
    int segments = 0;

    bool[] isUsed = new bool[marble.Length];

    while (idx < marble.Length && isUsed[idx] == false)
    {
        isUsed[idx] = true;
        idx = GetNextIndex(marble, idx, ref segments, ref teleports);
    }

    return idx >= marble.Length;
}

int GetNextIndex(string text, int idx, ref int segments, ref int teleports)
{
    switch (text[idx])
    {
        case '<':
...
bool IsTeleportChar(char ch)
{
    return char.IsDigit(ch) || ch == 'x' || (ch >= 'a' && ch <= 'f');
}

int GetTeleportIdx(string text, int idx)
{
    while (IsTeleportChar(text[idx - 1]))
    {
        idx--;
    }
    ...
    while (IsTeleportChar(text[idx + length]))
    {
        length++;
    }

    string teleport = text.Substring(idx, length);

    if (teleport.StartsWith("0x"))
    {
        idx = int.Parse(teleport.Substring(2), System.Globalization.NumberStyles.AllowHexSpecifier);
    }
    else
    {
        idx = int.Parse(teleport);
    }
