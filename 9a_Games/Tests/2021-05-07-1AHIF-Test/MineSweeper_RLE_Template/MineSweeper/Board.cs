...
public class Board
{
    ...
    public static void SetText(int row, int col, string text, string color)
    {
        ...
        if (row < 0 || row >= _staticBoard._rows || col < 0 || col >= _staticBoard._cols) { return; }
    ...
    public static string GetText(int row, int col)
    {
        ...
        if (row < 0 || row >= _staticBoard._rows || col < 0 || col >= _staticBoard._cols) { return null; }
    ...
    {
        if (_staticBoard == null) return;
        for (int i = 0; i < _staticBoard._rows; i++)
        {
            for (int j = 0; j < _staticBoard._cols; j++)
            {
                SetText(i, j, "");
            }
        }
...
public class FormBoard : Form
{
    ...
    private void CreateCell(int row, int col)
    {
        ...
        _board[row, col].Click += (sender, args) =>
        {
            if (sender is TextBox)
            {
                ...
                int x = t.Location.X / CellWidth;
                int y = t.Location.Y / CellHeight;
    ...
    private TextBox CreateTextBox(Color backGround, string text)
    {
        ...
        textBox.Font = new Font("Microsoft Sans Serif", 10F, FontStyle.Regular, GraphicsUnit.Point, 0);
