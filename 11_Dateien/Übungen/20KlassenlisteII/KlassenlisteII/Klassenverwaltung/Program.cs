...
namespace Klassenverwaltung
{
    class Program
    {
        ...
        static void ReadNewStudent(Pupil[] pupils, int pupCnt)
        {
            ...
            pupils[pupCnt].PupilNr = Convert.ToInt32(Console.ReadLine());

            Console.Write("Vorname: ");
            pupils[pupCnt].FirstName = Console.ReadLine();

            Console.Write("Nachname: ");
            pupils[pupCnt].LastName = Console.ReadLine();

            Console.Write("Plz.: ");
            pupils[pupCnt].ZipCode = Convert.ToInt32(Console.ReadLine());
        }
        ...
        static void PrintList(Pupil[] list, int cnt)
        {
            ...
            for (int i = 0; i < cnt; i++)
            {
                Console.WriteLine("{0,-5:d2} {1,-20} {2,-20} {3,-8}", list[i].PupilNr, list[i].FirstName, list[i].LastName, list[i].ZipCode);
            }
        }
        ...
        static void PrintZipcode(Pupil[] list, int cnt)
        {
            ...
            for (int i = 0; i < cnt; i++)
            {
                ...
                {
                    if (list[i].ZipCode == zipCodes[j].ZipCode)
                    {
                        zipCodes[j].Cnt++;
                        found = true;
                    }
                ...
                {
                    zipCodes[zipCnt].ZipCode = list[i].ZipCode;
                    zipCodes[zipCnt].Cnt++;
                    zipCnt++;
                }
            }
        ...
        static void SortByPupilNr(ref Pupil[] list, int cnt)
        {
            for (int i = 0; i < cnt; i++)
            {
                ...
                {
                    if (list[i].PupilNr > list[j].PupilNr)
                    {
        ...
        static void SortByName(ref Pupil[] list, int cnt)
        {
            for (int i = 0; i < cnt; i++)
            {
                for (int j = i+1; j < cnt; j++)
                {
                    if (list[i].LastName.CompareTo(list[j].LastName) > 0)
                    {
                        Swap(ref list[i], ref list[j]);
                    }
                }
        ...
        static void WritePupilsToCsv(Pupil[] pupils, int pupilCnt)
        {
            ...
            {
                lines[i] = pupils[i].PupilNr + ";" + pupils[i].FirstName + ";" + pupils[i].LastName + ";" + pupils[i].ZipCode;
            }
        ...
        static int ReadPupilsFromCsv(Pupil[] pupils)
        {
            ...
            if (File.Exists(FILENAME))
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    pupils[i].PupilNr = Convert.ToInt32(cols[0]);
                    pupils[i].FirstName = cols[1];
                    pupils[i].LastName = cols[2];
                    pupils[i].ZipCode = Convert.ToInt32(cols[3]);
                }
                return lines.Length;
            }
