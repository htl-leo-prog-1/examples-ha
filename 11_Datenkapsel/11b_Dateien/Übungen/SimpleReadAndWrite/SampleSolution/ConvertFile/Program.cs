...
namespace ConvertFile
{
    class MainClass
    {
        ...
        {
            string[] GRADES = {"SGT", "GUT", "BEF", "GEN", "NGD"};

            for (int i = 0; i < fileContent.GetLength(0); i++)
            {
                int gradeIndex = Convert.ToInt32(fileContent[i, GRADE]) - 1;
                fileContent[i, GRADE] = GRADES[gradeIndex];
            }
            return fileContent;
        }
        ...
        {
            string[] lines = new string[fileContent.GetLength(0)];
            for (int i = 0; i < lines.Length; i++)
            {
                lines[i] = fileContent[i, LASTNAME] + ";" + fileContent[i, FIRSTNAME] + ";" + fileContent[i, GRADE];
            }
